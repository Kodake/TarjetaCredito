...
export class TarjetaCreditoComponent implements OnInit, OnDestroy {
  ...
  agregar() {
    ...
    this.tarjetaService.guardarTarjeta(tarjeta).subscribe(data => {
      this.toastr.success('Registro Agregado', 'La tarjeta fue agregada satisfactoriamente');
  ...
  editar() {
    ...
    this.tarjetaService.actualizarTarjeta(this.idTarjeta, tarjeta).subscribe(data => {
      this.toastr.success('Registro Actualizado', 'La tarjeta fue actualizada satisfactoriamente');
      this.tarjetaService.obtenerTarjetas();
      this.form.reset();
      this.idTarjeta = 0;
    }); 
